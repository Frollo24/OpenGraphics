...
namespace OpenGraphics
{
    ...
    {
        glm::mat4 transpose = glm::transpose(this->toGlmMatrix());
    ...
    {
        glm::mat4 translate = glm::translate(this->toGlmMatrix(), translation.toGlmVector());
    ...
    inline Matrix4x4& Matrix4x4::Rotate(const Vector3D& axis, float angle)
    {
        glm::mat4 rotate = glm::rotate(this->toGlmMatrix(), glm::radians(angle), axis.toGlmVector());
    ...
    {
        glm::mat4 scaled = glm::scale(this->toGlmMatrix(), scale.toGlmVector());
    ...
    {
        glm::mat4 lookAt = glm::lookAt(from.toGlmVector(), to.toGlmVector(), up.toGlmVector());
        return Matrix4x4(
    ...
    {
        glm::vec4 result = this->toGlmMatrix() * glm::vec4(vector.toGlmVector(), 0.0f);
        return Vector3D(result.x, result.y, result.z);
    }

    inline Vector3D Matrix4x4::MultiplyPoint(const Vector3D& point) const
    {
        glm::vec4 result = this->toGlmMatrix() * glm::vec4(point.toGlmVector(), 1.0f);
        return Vector3D(result.x, result.y, result.z);
    }
